...
class CarExpensesController extends Controller
{
    ...
    public static function resolvePreLinkExpensesSBase(Car $car, float $linkRate): int
    {
        if ($car->registration_pre_expenses_s !== null) {
            return (int) $car->registration_pre_expenses_s;
        }

        $linkedExpenses = self::resolveLinkedRegistrationExpenses($car);
        $expected = self::computeLinkedRegistrationTotal($linkedExpenses, $linkRate);

        return max(0, (int) ($car->expenses_s ?? 0) - $expected);
    }
