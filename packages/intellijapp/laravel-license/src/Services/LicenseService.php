...
class LicenseService
{
    ...
    public static function getCurrentLicense(): ?License
    {
        ...
        return Cache::remember($cacheKey, 3600, function () {
            ...
            // محاولة الحصول من ملف الترخيص (Offline Mode)
    ...
    /**
     * التحقق من الترخيص Offline
     */
    public static function verifyOffline(): bool
    {
        if (!config('license.enabled')) {
            return true;
        }

        $license = self::loadFromFile();
        return $license && $license->isValid();
    }
     ...
     * حفظ الترخيص في ملف
     */
    public static function saveToFile(License $license): bool
    {
        try {
     ...
     */
    public static function loadFromFile(): ?License
    {
        try {
            $licenseFile = config('license.license_file');
            ...
            // إنشاء جديد من الملف (إذا لم يكن موجوداً في DB)
    ...
    /**
     * الحصول على معلومات الترخيص Offline
     */
    public static function getOfflineLicenseInfo(): array
    {
        $license = self::loadFromFile();
        ...
        if (!$license) {
            return [
                'activated' => false,
                'message' => 'الترخيص غير مفعل (Offline Mode)'
            ];
        }
     ...
     */
    public static function syncWithCentralServer(): array
    {
        if (!config('license.sync_enabled') || !config('license.sync_server_url')) {
            return [
                'success' => false,
                'message' => 'المزامنة غير مفعلة أو لم يتم تحديد سيرفر المزامنة'
            ];
        }
        ...
        try {
            ...
            if (!$license) {
                return [
                    'success' => false,
                    'message' => 'لا يوجد ترخيص للمزامنة'
                ];
            }

            $syncUrl = rtrim(config('license.sync_server_url'), '/') . '/api/license/sync';
            $token = config('license.sync_api_token');
            ...
            if ($response->successful()) {
                ...
                // تحديث الترخيص من السيرفر المركزي إذا لزم الأمر
                if (isset($data['license']) && is_array($data['license'])) {
                    $license->update($data['license']);
                    Cache::forget('current_license');
                }
            ...
            return [
                'success' => false,
                'message' => 'فشلت المزامنة: ' . $response->body()
            ];

        } catch (Exception $e) {
            Log::error('License sync failed', [
                'error' => $e->getMessage()
            ]);
     ...
     */
    public static function pullFromCentralServer(): array
    {
        if (!config('license.sync_enabled') || !config('license.sync_server_url')) {
            return [
                'success' => false,
        ...
        try {
            $domain = self::getCurrentDomain();
            $fingerprint = self::getServerFingerprint();
            
            $syncUrl = rtrim(config('license.sync_server_url'), '/') . '/api/license/pull';
            $token = config('license.sync_api_token');
            ...
            if ($response->successful()) {
                ...
                if (isset($data['license']) && is_array($data['license'])) {
                    // البحث عن ترخيص موجود أو إنشاء جديد
                    $license = License::updateOrCreate(
                        [
                            'domain' => $domain,
                            'fingerprint' => $fingerprint,
                    ...
                    return [
                        'success' => true,
                        'message' => 'تم جلب الترخيص من السيرفر المركزي',
                        'license' => $license
                    ];
                }
            }
            ...
            return [
                'success' => false,
                'message' => 'فشل جلب الترخيص: ' . $response->body()
            ];

        } catch (Exception $e) {
            Log::error('License pull failed', [
                'error' => $e->getMessage()
            ]);

            return [
                'success' => false,
                'message' => 'حدث خطأ أثناء جلب الترخيص: ' . $e->getMessage()
            ];
        }
    }
     ...
     */
    public static function pushToCentralServer(): array
    {
        if (!config('license.sync_enabled') || !config('license.sync_server_url')) {
            return [
                'success' => false,
        ...
        try {
            ...
            if (!$license) {
                return [
                    'success' => false,
                    'message' => 'لا يوجد ترخيص للإرسال'
                ];
            }

            $syncUrl = rtrim(config('license.sync_server_url'), '/') . '/api/license/push';
            $token = config('license.sync_api_token');
            ...
            if ($response->successful()) {
                return [
                    'success' => true,
                    'message' => 'تم إرسال الترخيص إلى السيرفر المركزي بنجاح',
                    'data' => $response->json()
                ];
            }
            ...
            return [
                'success' => false,
                'message' => 'فشل إرسال الترخيص: ' . $response->body()
            ];

        } catch (Exception $e) {
            Log::error('License push failed', [
                'error' => $e->getMessage()
            ]);

            return [
                'success' => false,
                'message' => 'حدث خطأ أثناء إرسال الترخيص: ' . $e->getMessage()
            ];
        }
    }

    /**
     * المزامنة التلقائية (يتم استدعاؤها من Scheduled Task)
     */
    public static function autoSync(): void
    {
        ...
        $lastSync = Cache::get('license_last_sync');
        $syncInterval = config('license.sync_interval', 3600);

        if ($lastSync && (now()->timestamp - $lastSync) < $syncInterval) {
            return; // لم يحن وقت المزامنة بعد
        }

        // محاولة المزامنة
        $result = self::syncWithCentralServer();
        
        if ($result['success']) {
            Cache::put('license_last_sync', now()->timestamp, $syncInterval);
            Log::info('License auto-sync completed successfully');
        } else {
            Log::warning('License auto-sync failed', ['message' => $result['message']]);
        }
    }
}
