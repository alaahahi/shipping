      ...
      <div class="modal-wrapper max-h-[90vh]">
        <div class="modal-container dark:bg-gray-900 overflow-auto max-h-[90vh]">
          <div class="modal-header">
            ...
            <p v-if="details?.car" class="text-center text-sm text-gray-600 dark:text-gray-200 px-4">
          ...
          <div class="modal-body px-4 pb-4">
            <div v-if="loading" class="text-center py-8 text-gray-500 dark:text-gray-300">جاري التحميل...</div>
            ...
            <template v-else-if="details?.has_registration">
              ...
              <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-600">
                <table class="w-full text-sm text-center text-gray-700 dark:text-gray-100">
                  ...
                  <tbody>
                    <tr
                      ...
                      :key="expense.id"
                      class="border-t border-gray-200 dark:border-gray-600 even:bg-gray-50 dark:even:bg-gray-800/80"
                    >
                      <td class="px-2 py-2 text-gray-800 dark:text-gray-100">{{ expense.created }}</td>
                      <td class="px-2 py-2 text-green-700 dark:text-green-300 font-semibold">
                        {{ formatNumber(expense.amount_dollar) }}
                      </td>
                      <td class="px-2 py-2 text-blue-700 dark:text-blue-300 font-semibold">
                        {{ formatNumber(expense.amount_dinar) }}
                      </td>
                      <td class="px-2 py-2 text-right text-xs max-w-[10rem] text-gray-700 dark:text-gray-200">{{ expense.note }}</td>
                      <td class="px-2 py-2 text-gray-800 dark:text-gray-100">{{ expense.user?.name }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
