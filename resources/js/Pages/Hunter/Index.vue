...
function openModalUpdateCarsHunter(form={}){
  formData.value=form

  showModalUpdateCarsHunter.value = true;
}
...
function openModalAddCarsHunter(form={}) {
  formData.value={}
    showModalAddCarsHunter.value = true;
}
...
const getResultsCar = async ($state) => {
  ...
  try {
    ...
    const response = await axios.get(`api/getIndexCarHunter`, {
      params: {
        limit: 100,
        page: page,
        q: q,
...
function confirmUpdateCar(V) {
  showModalUpdateCarsHunter.value = false;

  axios.post('/api/updateCarsHunter',V)
  .then(response => {
...
function confirmDelCar(V) {
  axios.post('/api/delCarsHunterr',V)
  .then(response => {
                              ...
                              <tbody>
                                ...
                                <tr v-for="car in car" :key="car.id" :class="car.results == 0 ?'':car.results == 1 ?'bg-red-100 dark:bg-red-900':'bg-green-100 dark:bg-green-900'"  class="bg-white border-b dark:bg-gray-900 dark:border-gray-900 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <button
                                      ...
                                      @click="openModalUpdateCarsHunter(car)"
                                    >
                                     <edit />
                                    </button>
                                    <button
