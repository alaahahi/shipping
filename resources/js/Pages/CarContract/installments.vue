...
async function savePayment() {
  ...
  saving.value = true;
  try {
    await axios.post('/api/contract-installments', {
  ...
  <AuthenticatedLayout>
    <div class="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
      <div class="mb-6">
        ...
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">عقود لم تُدفع بالكامل عند الإنشاء — تتبع مستقل مع وصل طباعة.</p>
      </div>

      <div class="bg-white dark:bg-gray-900 rounded-xl border border-slate-200 dark:border-slate-700 p-4 mb-4 flex flex-wrap gap-3 items-end">
      ...
      <div class="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-900">
          ...
          <table class="min-w-full text-xs">
            ...
            <tbody>
              <tr v-for="item in installments" :key="item.id" class="border-t border-slate-200 dark:border-slate-700">
                ...
                <td class="px-2 py-2">
                  <a :href="`/contract_installment_print/${item.id}`" target="_blank" class="text-sky-600 hover:underline">طباعة</a>
                </td>
              </tr>
            </tbody>
          </table>
